...
<?php 
	...
	if($_SERVER['REQUEST_METHOD'] == 'PUT')
	{
		...
		if(!empty($data['student_name']) AND !empty($data['age']) AND !empty($data['city']) AND !empty($data['id']))
		{
			if($stu->update_student() != FALSE)
			{
				...
				echo json_encode(array(
					'status' => 'success',
					'message' => "Data Update sucessfully SUccessfully"
				));
			}
			else{
